...
class Site extends CI_Controller {
	...
	public function menjadireseller(){
		...
		return $this->load->view('site/menjadireseller', $data);
	}
	...
	private function cookiesetter($kode = 0){
		if(!isset($_COOKIE[$kode])){
			...
			if($result == 1){
				setcookie($kode,"http://dropspaceindonesia.com", time()+3600);
			}
		}
	}
	...
	private function countervisitor(){
		...
		if($data_visitor == NULL){
			...
			$this->session->set_userdata('dropspaceindonesia',"Dhani");
			setcookie("dropspaceindonesia",'Dhani', time()+3600*24);
		}else{
			if(!isset($_COOKIE['dropspaceindonesia'])){
				$data_visitor = $this->model->GetVisitor("where ip = '".$_SERVER['REMOTE_ADDR']."' and tanggal = '".date("Y-m-d H:i:s")."'");
				if($data_visitor != NULL){
					if(!$this->session->userdata('dropspaceindonesia.com')){
						$data = array(
							'ip' => $_SERVER['REMOTE_ADDR'],
							'os' => $this->agent->platform(),
							'browser' => $agent,
					...
					$dhaniLocaltimezone = 'UTC';	$dhaniLocaltimezone = date_default_timezone_get();
					$data = array(
							'ip' => $_SERVER['REMOTE_ADDR'],
							'os' => $this->agent->platform(),
							'browser' => $agent,
